        ...
        <table class="table first-td-padding table-striped">
          ...
          <tbody>
            <tr v-if="enrollments_skipped.length" v-for="enrollment in enrollments_skipped" :key='enrollment.id' :request="enrollment" v-bind:class="{skipped:enrollment.status == 'skipped'}">
              <td>
                <input v-show="checked===true" type="checkbox" v-model="selected" :value="enrollment.id" number>{{ enrollment.id }}
              </td>
              <td>
              ...
              <button data-toggle="modal" data-target="#smsModal3" class="btn btn-primary">Отправить смс</button>
      <!-- Modal -->
<div id="smsModal3" class="mt-5 modal fade" role="dialog">
  ...
  <b-collapse id="collapse-1" class="mt-2">
      <p v-for="enrollment in enrollments_skipped"> <input type="checkbox" v-model="selected" :value="enrollment.id" number>{{ users[enrollment.user_id].name }}</p>
  </b-collapse>
          </div>
        </div>
  ...
  computed: {
    sourceData (i) {
      ...
      for (i = 0; i < lengthArr; i++) {
        for (let j=0; j < this.enrollments_skipped.length; j++){
        if(this.enrollments_skipped[j].id===this.selected[i]){
          let user = this.enrollments_skipped[j].user_id
    ...
    selectAll: {
      get: function () {
        return this.enrollments_skipped ? this.selected.length == this.enrollments_skipped.length : false;
      },
      set: function (value) {
        let selectedSms = []
        var selected = [];
        if (value) {
          this.enrollments_skipped.forEach(function (enrollment) {
