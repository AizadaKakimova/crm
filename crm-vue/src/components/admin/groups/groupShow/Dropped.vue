  ...
  <div>
    ...
    <download-excel
        ...
        <table class="table first-td-padding table-striped">
          ...
          <tbody>
            <tr v-if="enrollments_dropped.length" v-for="enrollment in enrollments_dropped" :key='enrollment.id' :request="enrollment" v-bind:class="{enrolled:enrollment.status == ''}">
              <td>
            ...
            <!-- <tr v-if="enrollments_dropped.length" v-for="enrollment in enrollments_dropped" :key='enrollment.id' :request="enrollment" v-bind:class="{dropped:enrollment.status == 'dropped'}">
              <td>
                <input class="mr-2" v-show="checked===true" type="checkbox" v-model="selected" :value="enrollment.id" number>{{ enrollment.id }}
              </td>
              <td>
                <router-link :to="`/admin/groups/${group.id}/enrollments/${enrollment.id}`">{{ users[enrollment.user_id].name }}</router-link>
              </td>
              <td>
                {{ users[enrollment.user_id].phone }}
              </td>
            ...
            <button data-toggle="modal" data-target="#smsModal2" class="btn btn-primary">Отправить смс</button>
      <!-- Modal -->
<div id="smsModal2" class="mt-5 modal fade" role="dialog">
  ...
  <b-collapse id="collapse-1" class="mt-2">
      <p v-for="enrollment in enrollments_dropped"> <input type="checkbox" v-model="selected" :value="enrollment.id" number>{{ users[enrollment.user_id].name }}</p>
  </b-collapse>
          </div>
        </div>
...
export default {
  ...
  mounted: {
    ...
    dropped() {
      return enrollments_dropped.length
    }
  },
  computed: {
    sourceData (i) {
      ...
      // alert(this.enrollments_dropped.length)
      let en 
      for (i = 0; i < lengthArr; i++) {
        for (let j=0; j < this.enrollments_dropped.length; j++){
        if(this.enrollments_dropped[j].id===this.selected[i]){
          let user = this.enrollments_dropped[j].user_id
    ...
    selectAll: {
      get: function () {
        return this.enrollments_dropped ? this.selected.length == this.enrollments_dropped.length : false;
      },
      set: function (value) {
        let selectedSms = []
        var selected = [];
        if (value) {
          this.enrollments_dropped.forEach(function (enrollment) {
      ...
      if (this.$store.state.signedIn && this.$store.getters.isAdmin) {
        this.$http.secured.get(`/${this.$store.getters.getCurrentCity.slug}/admin/groups/${this.$route.params.slug}`)
          .then(response => {
            ...
            for(let i=0; i<this.enrollments_dropped.length;i++) {
              this.json_data[i] = {}
              this.json_data[i] = this.users[this.enrollments_dropped[i].user_id]
